...
<div class="card card-body">
  ...
  <form id="form" class="row" action="<?= base_url('dashboard/project') ?>" method="POST" enctype="multipart/form-data">
    <div class="col-12 col-md-8">
      <div class="mb-3">
        <label for="name" class="form-label small required">Project Name</label>
        <input type="text" class="form-control form-control-sm rounded-0 <?= isset(session('validation')['name']) ? 'is-invalid' : '' ?>" name="name" id="name" value="<?= old('name') ?>">
        <?php if (isset(session('validation')['name'])) : ?>
          <div class="invalid-feedback"><?= session('validation')['name'] ?></div>
        <?php endif ?>
      </div>
      <div class="mb-3">
        ...
        <select class="form-select form-select-sm rounded-0 <?= isset(session('validation')['category_id']) ? 'is-invalid' : '' ?>" name="category_id">
          ...
          <?php foreach ($project_categories as $project_category) : ?>
            <option <?= old('category_id') == $project_category['id'] ? 'selected' : '' ?> value="<?= $project_category['id'] ?>"><?= $project_category['name'] ?></option>
          <?php endforeach ?>
        </select>
      ...
      <div class="mb-3">
        ...
        <textarea class="form-control form-control-sm rounded-0 <?= isset(session('validation')['description']) ? 'is-invalid' : '' ?>" name="description" id="description" rows="4"><?= old('description') ?></textarea>
      ...
      <div class="row g-2 mb-3">
        <div class="col-6">
          ...
          <input type="url" placeholder="https://example.com" class="form-control form-control-sm rounded-0 <?= isset(session('validation')['link']) ? 'is-invalid' : '' ?>" name="link" id="link" value="<?= old('link') ?>">
        ...
        <div class="col-6">
          ...
          <input type="url" placeholder="https://github.com/example" class="form-control form-control-sm rounded-0 <?= isset(session('validation')['link_github']) ? 'is-invalid' : '' ?>" name="link_github" id="link_github" value="<?= old('link_github') ?>">
      ...
      <div class="form-check">
        <input class="form-check-input rounded-0" type="checkbox" name="is_featured" id="is_featured" <?= old('is_featured') == 'on' ? 'checked' : '' ?>>
    ...
    <div class="col-12 col-md-4">
      <div class="mb-3">
        <label for="photo" class="form-label small required">Project Image</label>
        ...
        <?php if (isset(session('validation')['photo'])) : ?><div class="invalid-feedback"><?= session('validation')['photo'] ?></div><?php endif ?>
      </div>
      <div class="mb-3" id="image_preview_container" style="display: none;">
        ...
        <div id="image_preview" style="position: relative; width: 100%; height: 400px; overflow: hidden; text-align: center;">
          <img id="preview_image" style="max-width: 100%; max-height: 100%; display: inline-block; vertical-align: middle;">
        </div>
      </div>
    </div>
