...
class ContactController extends BaseController
{
  ...
  {
    $data['title']    = 'Contact - ' . setting()->get('App.sitename');
    $data['contacts'] = $this->model->orderBy('created_at', 'DESC')->paginate(30, 'contact');
  ...
  public function detail($id)
  {
    ...
    if ($data) {
      $data['timestamp'] = \Carbon\Carbon::parse($data['timestamp'])->timezone('Asia/Jakarta')->format('d M Y h:i A');
      $response = [
        'status'  => 200,
        'data'    => $data
      ];
